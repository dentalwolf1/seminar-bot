...
def _auth_header():
    ...
    return {
        "Authorization": f"HMAC-SHA256 apiKey={SOLAPI_API_KEY}, date={now}, salt={salt}, signature={sig}",
...
def send_sms(phone, text):
    resp = requests.post(
        ...
        json={"message": {"to": phone, "from": SOLAPI_SENDER, "text": text, "type": "SMS"}},
        timeout=10
    )
...
# ---------------------------------------------------------------------------
# ★ INPUT FORMAT (must start with "세미나예약 ")
#
#   세미나예약 01012345678 06/10
# ---------------------------------------------------------------------------
...
INPUT_PATTERN = re.compile(r'^세미나예약\s+(01\d{8,9})\s+(\d{1,2})[/월]\s*(\d{1,2})일?\s*$')
